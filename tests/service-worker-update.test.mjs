...
test('o app atualiza o Service Worker instalado sem exigir limpeza manual', () => {
  assert.match(indexSource, /script\.js\?v=2\.1\.17/u);
  assert.match(scriptSource, /SERVICE_WORKER_VERSION\s*=\s*['"]2\.1\.17['"]/u);
  ...
  assert.match(serviceWorkerSource, /CACHE_NAME\s*=\s*['"]aprendizagem-cache-v24['"]/u);
  assert.match(serviceWorkerSource, /\.\/data\/country-curiosities\.pt-BR\.json/u);
  assert.match(buildValidatorSource, /script\\.js\\.?v=2\\.1\\.17/u);
  assert.match(buildValidatorSource, /aprendizagem-cache-v24/u);
