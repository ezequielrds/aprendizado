...
test('o app atualiza o Service Worker instalado sem exigir limpeza manual', () => {
  assert.match(indexSource, /script\.js\?v=2\.1\.8/u);
  assert.match(scriptSource, /SERVICE_WORKER_VERSION\s*=\s*['"]2\.1\.8['"]/u);
  ...
  assert.match(serviceWorkerSource, /CACHE_NAME\s*=\s*['"]aprendizagem-cache-v15['"]/u);
